...
from roopfusion import logger, process_manager
from roopfusion.typing import OutputVideoPreset, Fps, AudioBuffer
from roopfusion.filesystem import get_temp_frames_pattern, get_temp_file_path
from roopfusion.vision import restrict_video_fps
...
def run_ffmpeg(args : List[str]) -> bool:
	...
	while process_manager.is_processing():
		try:
			if roopfusion.globals.log_level == 'debug':
				log_debug(process)
...
def extract_frames(target_path : str, temp_video_resolution : str, temp_video_fps : Fps) -> bool:
	trim_frame_start = roopfusion.globals.trim_frame_start
	trim_frame_end = roopfusion.globals.trim_frame_end
...
def merge_video(target_path : str, output_video_resolution : str, output_video_fps : Fps) -> bool:
	...
	temp_frames_pattern = get_temp_frames_pattern(target_path, '%04d')
	commands = [ '-r', str(temp_video_fps), '-i', temp_frames_pattern, '-s', str(output_video_resolution), '-c:v', roopfusion.globals.output_video_encoder ]

	if roopfusion.globals.output_video_encoder in [ 'libx264', 'libx265' ]:
		output_video_compression = round(51 - (roopfusion.globals.output_video_quality * 0.51))
		commands.extend([ '-crf', str(output_video_compression), '-preset', roopfusion.globals.output_video_preset ])
	if roopfusion.globals.output_video_encoder in [ 'libvpx-vp9' ]:
		output_video_compression = round(63 - (roopfusion.globals.output_video_quality * 0.63))
		commands.extend([ '-crf', str(output_video_compression) ])
	if roopfusion.globals.output_video_encoder in [ 'h264_nvenc', 'hevc_nvenc' ]:
		output_video_compression = round(51 - (roopfusion.globals.output_video_quality * 0.51))
		commands.extend([ '-cq', str(output_video_compression), '-preset', map_nvenc_preset(roopfusion.globals.output_video_preset) ])
	if roopfusion.globals.output_video_encoder in [ 'h264_amf', 'hevc_amf' ]:
		output_video_compression = round(51 - (roopfusion.globals.output_video_quality * 0.51))
		commands.extend([ '-qp_i', str(output_video_compression), '-qp_p', str(output_video_compression), '-quality', map_amf_preset(roopfusion.globals.output_video_preset) ])
...
def finalize_image(target_path : str, output_path : str, output_image_resolution : str) -> bool:
	temp_file_path = get_temp_file_path(target_path)
	output_image_compression = round(31 - (roopfusion.globals.output_image_quality * 0.31))
