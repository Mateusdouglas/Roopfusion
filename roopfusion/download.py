...
from roopfusion.common_helper import is_macos
from roopfusion.filesystem import get_file_size, is_file
...
def conditional_download(download_directory_path : str, urls : List[str]) -> None:
	for url in urls:
		...
		if initial_size < download_size:
			with tqdm(total = download_size, initial = initial_size, desc = wording.get('downloading'), unit = 'B', unit_scale = True, unit_divisor = 1024, ascii = ' =', disable = roopfusion.globals.log_level in [ 'warn', 'error' ]) as progress:
