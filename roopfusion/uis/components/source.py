...
from roopfusion.filesystem import has_audio, has_image, filter_audio_paths, filter_image_paths
...
def render() -> None:
	...
	has_source_audio = has_audio(roopfusion.globals.source_paths)
	has_source_image = has_image(roopfusion.globals.source_paths)
	SOURCE_FILE = gradio.File(
		...
		value = roopfusion.globals.source_paths if has_source_audio or has_source_image else None
	)
...
def update(files : List[File]) -> Tuple[gradio.Audio, gradio.Image]:
	...
	if has_source_audio or has_source_image:
		...
		roopfusion.globals.source_paths = file_names
	...
	roopfusion.globals.source_paths = None
