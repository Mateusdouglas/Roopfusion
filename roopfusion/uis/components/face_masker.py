...
from roopfusion.typing import FaceMaskType, FaceMaskRegion
...
def render() -> None:
	...
	has_box_mask = 'box' in roopfusion.globals.face_mask_types
	has_region_mask = 'region' in roopfusion.globals.face_mask_types
	FACE_MASK_TYPES_CHECKBOX_GROUP = gradio.CheckboxGroup(
		label = wording.get('uis.face_mask_types_checkbox_group'),
		choices = roopfusion.choices.face_mask_types,
		value = roopfusion.globals.face_mask_types
	)
	with gradio.Group(visible = has_box_mask) as FACE_MASK_BOX_GROUP:
		FACE_MASK_BLUR_SLIDER = gradio.Slider(
			...
			step = roopfusion.choices.face_mask_blur_range[1] - roopfusion.choices.face_mask_blur_range[0],
			minimum = roopfusion.choices.face_mask_blur_range[0],
			maximum = roopfusion.choices.face_mask_blur_range[-1],
			value = roopfusion.globals.face_mask_blur
		)
		with gradio.Row():
			FACE_MASK_PADDING_TOP_SLIDER = gradio.Slider(
				label = wording.get('uis.face_mask_padding_top_slider'),
				step = roopfusion.choices.face_mask_padding_range[1] - roopfusion.choices.face_mask_padding_range[0],
				minimum = roopfusion.choices.face_mask_padding_range[0],
				maximum = roopfusion.choices.face_mask_padding_range[-1],
				value = roopfusion.globals.face_mask_padding[0]
			)
			FACE_MASK_PADDING_RIGHT_SLIDER = gradio.Slider(
				...
				minimum = roopfusion.choices.face_mask_padding_range[0],
				maximum = roopfusion.choices.face_mask_padding_range[-1],
				value = roopfusion.globals.face_mask_padding[1]
			)
		with gradio.Row():
			FACE_MASK_PADDING_BOTTOM_SLIDER = gradio.Slider(
				...
				minimum = roopfusion.choices.face_mask_padding_range[0],
				maximum = roopfusion.choices.face_mask_padding_range[-1],
				value = roopfusion.globals.face_mask_padding[2]
			)
			FACE_MASK_PADDING_LEFT_SLIDER = gradio.Slider(
				...
				minimum = roopfusion.choices.face_mask_padding_range[0],
				maximum = roopfusion.choices.face_mask_padding_range[-1],
				value = roopfusion.globals.face_mask_padding[3]
			)
	with gradio.Row():
		FACE_MASK_REGION_CHECKBOX_GROUP = gradio.CheckboxGroup(
			label = wording.get('uis.face_mask_region_checkbox_group'),
			choices = roopfusion.choices.face_mask_regions,
			value = roopfusion.globals.face_mask_regions,
...
def update_face_mask_type(face_mask_types : List[FaceMaskType]) -> Tuple[gradio.CheckboxGroup, gradio.Group, gradio.CheckboxGroup]:
	roopfusion.globals.face_mask_types = face_mask_types or roopfusion.choices.face_mask_types
	has_box_mask = 'box' in face_mask_types
	has_region_mask = 'region' in face_mask_types
	return gradio.CheckboxGroup(value = roopfusion.globals.face_mask_types), gradio.Group(visible = has_box_mask), gradio.CheckboxGroup(visible = has_region_mask)


def update_face_mask_blur(face_mask_blur : float) -> None:
	roopfusion.globals.face_mask_blur = face_mask_blur
...
def update_face_mask_padding(face_mask_padding_top : int, face_mask_padding_right : int, face_mask_padding_bottom : int, face_mask_padding_left : int) -> None:
	roopfusion.globals.face_mask_padding = (face_mask_padding_top, face_mask_padding_right, face_mask_padding_bottom, face_mask_padding_left)


def update_face_mask_regions(face_mask_regions : List[FaceMaskRegion]) -> gradio.CheckboxGroup:
	roopfusion.globals.face_mask_regions = face_mask_regions or roopfusion.choices.face_mask_regions
	return gradio.CheckboxGroup(value = roopfusion.globals.face_mask_regions)
