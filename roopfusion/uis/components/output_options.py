...
from roopfusion.typing import OutputVideoEncoder, OutputVideoPreset, Fps
from roopfusion.filesystem import is_image, is_video
from roopfusion.uis.core import get_ui_components, register_ui_component
from roopfusion.vision import detect_image_resolution, create_image_resolutions, detect_video_fps, detect_video_resolution, create_video_resolutions, pack_resolution
...
def render() -> None:
	...
	if is_image(roopfusion.globals.target_path):
		output_image_resolution = detect_image_resolution(roopfusion.globals.target_path)
		output_image_resolutions = create_image_resolutions(output_image_resolution)
	if is_video(roopfusion.globals.target_path):
		output_video_resolution = detect_video_resolution(roopfusion.globals.target_path)
		output_video_resolutions = create_video_resolutions(output_video_resolution)
	roopfusion.globals.output_path = roopfusion.globals.output_path or '.'
	OUTPUT_PATH_TEXTBOX = gradio.Textbox(
		label = wording.get('uis.output_path_textbox'),
		value = roopfusion.globals.output_path,
		max_lines = 1
	)
	OUTPUT_IMAGE_QUALITY_SLIDER = gradio.Slider(
		label = wording.get('uis.output_image_quality_slider'),
		value = roopfusion.globals.output_image_quality,
		step = roopfusion.choices.output_image_quality_range[1] - roopfusion.choices.output_image_quality_range[0],
		minimum = roopfusion.choices.output_image_quality_range[0],
		maximum = roopfusion.choices.output_image_quality_range[-1],
		visible = is_image(roopfusion.globals.target_path)
	)
	OUTPUT_IMAGE_RESOLUTION_DROPDOWN = gradio.Dropdown(
		...
		value = roopfusion.globals.output_image_resolution,
		visible = is_image(roopfusion.globals.target_path)
	)
	OUTPUT_VIDEO_ENCODER_DROPDOWN = gradio.Dropdown(
		label = wording.get('uis.output_video_encoder_dropdown'),
		choices = roopfusion.choices.output_video_encoders,
		value = roopfusion.globals.output_video_encoder,
		visible = is_video(roopfusion.globals.target_path)
	)
	OUTPUT_VIDEO_PRESET_DROPDOWN = gradio.Dropdown(
		label = wording.get('uis.output_video_preset_dropdown'),
		choices = roopfusion.choices.output_video_presets,
		value = roopfusion.globals.output_video_preset,
		visible = is_video(roopfusion.globals.target_path)
	)
	OUTPUT_VIDEO_QUALITY_SLIDER = gradio.Slider(
		label = wording.get('uis.output_video_quality_slider'),
		value = roopfusion.globals.output_video_quality,
		step = roopfusion.choices.output_video_quality_range[1] - roopfusion.choices.output_video_quality_range[0],
		minimum = roopfusion.choices.output_video_quality_range[0],
		maximum = roopfusion.choices.output_video_quality_range[-1],
		visible = is_video(roopfusion.globals.target_path)
	)
	OUTPUT_VIDEO_RESOLUTION_DROPDOWN = gradio.Dropdown(
		...
		value = roopfusion.globals.output_video_resolution,
		visible = is_video(roopfusion.globals.target_path)
	)
	OUTPUT_VIDEO_FPS_SLIDER = gradio.Slider(
		label = wording.get('uis.output_video_fps_slider'),
		value = roopfusion.globals.output_video_fps,
		step = 0.01,
		minimum = 1,
		maximum = 60,
		visible = is_video(roopfusion.globals.target_path)
...
def update_output_path(output_path : str) -> None:
	roopfusion.globals.output_path = output_path


def update_output_image_quality(output_image_quality : int) -> None:
	roopfusion.globals.output_image_quality = output_image_quality


def update_output_image_resolution(output_image_resolution : str) -> None:
	roopfusion.globals.output_image_resolution = output_image_resolution


def update_output_video_encoder(output_video_encoder: OutputVideoEncoder) -> None:
	roopfusion.globals.output_video_encoder = output_video_encoder


def update_output_video_preset(output_video_preset : OutputVideoPreset) -> None:
	roopfusion.globals.output_video_preset = output_video_preset


def update_output_video_quality(output_video_quality : int) -> None:
	roopfusion.globals.output_video_quality = output_video_quality


def update_output_video_resolution(output_video_resolution : str) -> None:
	roopfusion.globals.output_video_resolution = output_video_resolution


def update_output_video_fps(output_video_fps : Fps) -> None:
	roopfusion.globals.output_video_fps = output_video_fps
