...
from roopfusion.processors.frame.core import load_frame_processor_module, clear_frame_processors_modules
from roopfusion.filesystem import list_directory
...
def render() -> None:
	...
	FRAME_PROCESSORS_CHECKBOX_GROUP = gradio.CheckboxGroup(
		...
		choices = sort_frame_processors(roopfusion.globals.frame_processors),
		value = roopfusion.globals.frame_processors
	)
...
def update_frame_processors(frame_processors : List[str]) -> gradio.CheckboxGroup:
	roopfusion.globals.frame_processors = frame_processors
	clear_frame_processors_modules()
	for frame_processor in frame_processors:
		frame_processor_module = load_frame_processor_module(frame_processor)
		if not frame_processor_module.pre_check():
			return gradio.CheckboxGroup()
	return gradio.CheckboxGroup(value = roopfusion.globals.frame_processors, choices = sort_frame_processors(roopfusion.globals.frame_processors))
