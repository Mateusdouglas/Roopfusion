...
from roopfusion.uis.typing import File
from roopfusion.filesystem import get_file_size, is_image, is_video
from roopfusion.uis.core import register_ui_component
from roopfusion.vision import get_video_frame, normalize_frame_color
...
def render() -> None:
	...
	is_target_image = is_image(roopfusion.globals.target_path)
	is_target_video = is_video(roopfusion.globals.target_path)
	TARGET_FILE = gradio.File(
		...
		value = roopfusion.globals.target_path if is_target_image or is_target_video else None
	)
	target_image_args =\
	{
		'show_label': False,
	...
	if is_target_video:
		if get_file_size(roopfusion.globals.target_path) > FILE_SIZE_LIMIT:
			preview_vision_frame = normalize_frame_color(get_video_frame(roopfusion.globals.target_path))
...
def update(file : File) -> Tuple[gradio.Image, gradio.Video]:
	...
	if file and is_image(file.name):
		roopfusion.globals.target_path = file.name
	...
	roopfusion.globals.target_path = None
