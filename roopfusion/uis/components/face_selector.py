...
from roopfusion.face_store import clear_static_faces, clear_reference_faces
from roopfusion.vision import get_video_frame, read_static_image, normalize_frame_color
from roopfusion.filesystem import is_image, is_video
from roopfusion.face_analyser import get_many_faces
from roopfusion.typing import VisionFrame, FaceSelectorMode
...
def render() -> None:
	...
	reference_face_gallery_args : Dict[str, Any] =\
	{
		...
		'columns': 8,
		'allow_preview': False,
		'visible': 'reference' in roopfusion.globals.face_selector_mode
	}
	if is_image(roopfusion.globals.target_path):
		reference_frame = read_static_image(roopfusion.globals.target_path)
	...
	FACE_SELECTOR_MODE_DROPDOWN = gradio.Dropdown(
		...
		choices = roopfusion.choices.face_selector_modes,
		value = roopfusion.globals.face_selector_mode
	)
	REFERENCE_FACE_POSITION_GALLERY = gradio.Gallery(**reference_face_gallery_args)
	REFERENCE_FACE_DISTANCE_SLIDER = gradio.Slider(
		label = wording.get('uis.reference_face_distance_slider'),
		value = roopfusion.globals.reference_face_distance,
		step = roopfusion.choices.reference_face_distance_range[1] - roopfusion.choices.reference_face_distance_range[0],
		minimum = roopfusion.choices.reference_face_distance_range[0],
		maximum = roopfusion.choices.reference_face_distance_range[-1],
		visible = 'reference' in roopfusion.globals.face_selector_mode
	)
...
def update_face_selector_mode(face_selector_mode : FaceSelectorMode) -> Tuple[gradio.Gallery, gradio.Slider]:
	if face_selector_mode == 'many':
		roopfusion.globals.face_selector_mode = face_selector_mode
...
def update_reference_face_position(reference_face_position : int = 0) -> None:
	roopfusion.globals.reference_face_position = reference_face_position


def update_reference_face_distance(reference_face_distance : float) -> None:
	roopfusion.globals.reference_face_distance = reference_face_distance


def update_reference_frame_number(reference_frame_number : int) -> None:
	roopfusion.globals.reference_frame_number = reference_frame_number
...
def update_reference_position_gallery() -> gradio.Gallery:
	...
	if is_image(roopfusion.globals.target_path):
		temp_vision_frame = read_static_image(roopfusion.globals.target_path)
