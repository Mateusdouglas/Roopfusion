...
from roopfusion import logger, wording
from roopfusion.audio import get_audio_frame, create_empty_audio_frame
from roopfusion.common_helper import get_first
from roopfusion.core import conditional_append_reference_faces
from roopfusion.face_analyser import get_average_face, clear_face_analyser
from roopfusion.face_store import clear_static_faces, get_reference_faces, clear_reference_faces
from roopfusion.typing import Face, FaceSet, AudioFrame, VisionFrame
from roopfusion.vision import get_video_frame, count_video_frame_total, normalize_frame_color, resize_frame_resolution, read_static_image, read_static_images
from roopfusion.filesystem import is_image, is_video, filter_audio_paths
from roopfusion.content_analyser import analyse_frame
from roopfusion.processors.frame.core import load_frame_processor_module
from roopfusion.uis.core import get_ui_component, get_ui_components, register_ui_component
...
def render() -> None:
	...
	if source_audio_path and roopfusion.globals.output_video_fps and roopfusion.globals.reference_frame_number:
		temp_audio_frame = get_audio_frame(source_audio_path, roopfusion.globals.output_video_fps, roopfusion.globals.reference_frame_number)
		if numpy.any(temp_audio_frame):
			source_audio_frame = temp_audio_frame
	if is_image(roopfusion.globals.target_path):
		target_vision_frame = read_static_image(roopfusion.globals.target_path)
	...
	if is_video(roopfusion.globals.target_path):
		temp_vision_frame = get_video_frame(roopfusion.globals.target_path, roopfusion.globals.reference_frame_number)
		preview_vision_frame = process_preview_frame(reference_faces, source_face, source_audio_frame, temp_vision_frame)
		preview_image_args['value'] = normalize_frame_color(preview_vision_frame)
		preview_image_args['visible'] = True
		preview_frame_slider_args['value'] = roopfusion.globals.reference_frame_number
		preview_frame_slider_args['maximum'] = count_video_frame_total(roopfusion.globals.target_path)
...
def update_preview_image(frame_number : int = 0) -> gradio.Image:
	for frame_processor in roopfusion.globals.frame_processors:
	...
	source_audio_frame = create_empty_audio_frame()
	if source_audio_path and roopfusion.globals.output_video_fps and roopfusion.globals.reference_frame_number:
		reference_audio_frame_number = roopfusion.globals.reference_frame_number
		if roopfusion.globals.trim_frame_start:
			reference_audio_frame_number -= roopfusion.globals.trim_frame_start
		temp_audio_frame = get_audio_frame(source_audio_path, roopfusion.globals.output_video_fps, reference_audio_frame_number)
		if numpy.any(temp_audio_frame):
			source_audio_frame = temp_audio_frame
	if is_image(roopfusion.globals.target_path):
		target_vision_frame = read_static_image(roopfusion.globals.target_path)
	...
	if is_video(roopfusion.globals.target_path):
		temp_vision_frame = get_video_frame(roopfusion.globals.target_path, frame_number)
		preview_vision_frame = process_preview_frame(reference_faces, source_face, source_audio_frame, temp_vision_frame)
		preview_vision_frame = normalize_frame_color(preview_vision_frame)
		return gradio.Image(value = preview_vision_frame)
...
def update_preview_frame_slider() -> gradio.Slider:
	if is_video(roopfusion.globals.target_path):
		video_frame_total = count_video_frame_total(roopfusion.globals.target_path)
