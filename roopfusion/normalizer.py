...
from roopfusion.filesystem import is_directory
from roopfusion.typing import Padding, Fps
...
def normalize_output_path(target_path : Optional[str], output_path : Optional[str]) -> Optional[str]:
	if target_path and output_path:
		...
		if is_directory(output_path):
			output_hash = hashlib.sha1(str(roopfusion.globals.__dict__).encode('utf-8')).hexdigest()[:8]
