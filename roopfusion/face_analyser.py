...
from roopfusion import process_manager
from roopfusion.common_helper import get_first
from roopfusion.face_helper import estimate_matrix_by_face_landmark_5, warp_face_by_face_landmark_5, warp_face_by_translation, create_static_anchors, distance_to_face_landmark_5, distance_to_bounding_box, convert_face_landmark_68_to_5, apply_nms, categorize_age, categorize_gender
from roopfusion.face_store import get_static_faces, set_static_faces
from roopfusion.execution import apply_execution_provider_options
from roopfusion.download import conditional_download
from roopfusion.filesystem import resolve_relative_path, is_file
from roopfusion.thread_helper import thread_lock, thread_semaphore, conditional_thread_semaphore
from roopfusion.typing import VisionFrame, Face, FaceSet, FaceAnalyserOrder, FaceAnalyserAge, FaceAnalyserGender, ModelSet, BoundingBox, FaceLandmarkSet, FaceLandmark5, FaceLandmark68, Score, FaceScoreSet, Embedding
from roopfusion.vision import resize_frame_resolution, unpack_resolution
...
def get_face_analyser() -> Any:
	...
	with thread_lock():
		...
		if FACE_ANALYSER is None:
			if roopfusion.globals.face_detector_model in [ 'many', 'retinaface' ]:
				face_detectors['retinaface'] = onnxruntime.InferenceSession(MODELS.get('face_detector_retinaface').get('path'), providers = apply_execution_provider_options(roopfusion.globals.execution_device_id, roopfusion.globals.execution_providers))
			if roopfusion.globals.face_detector_model in [ 'many', 'scrfd' ]:
				face_detectors['scrfd'] = onnxruntime.InferenceSession(MODELS.get('face_detector_scrfd').get('path'), providers = apply_execution_provider_options(roopfusion.globals.execution_device_id, roopfusion.globals.execution_providers))
			if roopfusion.globals.face_detector_model in [ 'many', 'yoloface' ]:
				face_detectors['yoloface'] = onnxruntime.InferenceSession(MODELS.get('face_detector_yoloface').get('path'), providers = apply_execution_provider_options(roopfusion.globals.execution_device_id, roopfusion.globals.execution_providers))
			if roopfusion.globals.face_detector_model in [ 'yunet' ]:
				face_detectors['yunet'] = cv2.FaceDetectorYN.create(MODELS.get('face_detector_yunet').get('path'), '', (0, 0))
			if roopfusion.globals.face_recognizer_model == 'arcface_blendswap':
				face_recognizer = onnxruntime.InferenceSession(MODELS.get('face_recognizer_arcface_blendswap').get('path'), providers = apply_execution_provider_options(roopfusion.globals.execution_device_id, roopfusion.globals.execution_providers))
			if roopfusion.globals.face_recognizer_model == 'arcface_inswapper':
				face_recognizer = onnxruntime.InferenceSession(MODELS.get('face_recognizer_arcface_inswapper').get('path'), providers = apply_execution_provider_options(roopfusion.globals.execution_device_id, roopfusion.globals.execution_providers))
			if roopfusion.globals.face_recognizer_model == 'arcface_simswap':
				face_recognizer = onnxruntime.InferenceSession(MODELS.get('face_recognizer_arcface_simswap').get('path'), providers = apply_execution_provider_options(roopfusion.globals.execution_device_id, roopfusion.globals.execution_providers))
			if roopfusion.globals.face_recognizer_model == 'arcface_uniface':
				face_recognizer = onnxruntime.InferenceSession(MODELS.get('face_recognizer_arcface_uniface').get('path'), providers = apply_execution_provider_options(roopfusion.globals.execution_device_id, roopfusion.globals.execution_providers))
			face_landmarkers['68'] = onnxruntime.InferenceSession(MODELS.get('face_landmarker_68').get('path'), providers = apply_execution_provider_options(roopfusion.globals.execution_device_id, roopfusion.globals.execution_providers))
			face_landmarkers['68_5'] = onnxruntime.InferenceSession(MODELS.get('face_landmarker_68_5').get('path'), providers = apply_execution_provider_options(roopfusion.globals.execution_device_id, roopfusion.globals.execution_providers))
			gender_age = onnxruntime.InferenceSession(MODELS.get('gender_age').get('path'), providers = apply_execution_provider_options(roopfusion.globals.execution_device_id, roopfusion.globals.execution_providers))
			FACE_ANALYSER =\
			{
...
def pre_check() -> bool:
	...
	if not roopfusion.globals.skip_download:
...
def detect_with_retinaface(vision_frame : VisionFrame, face_detector_size : str) -> Tuple[List[BoundingBox], List[FaceLandmark5], List[Score]]:
	...
	for index, feature_stride in enumerate(feature_strides):
		keep_indices = numpy.where(detections[index] >= roopfusion.globals.face_detector_score)[0]
		if keep_indices.any():
...
def detect_with_yoloface(vision_frame : VisionFrame, face_detector_size : str) -> Tuple[List[BoundingBox], List[FaceLandmark5], List[Score]]:
	...
	keep_indices = numpy.where(score_raw > roopfusion.globals.face_detector_score)[0]
	if keep_indices.any():
...
def detect_with_yunet(vision_frame : VisionFrame, face_detector_size : str) -> Tuple[List[BoundingBox], List[FaceLandmark5], List[Score]]:
	...
	face_detector.setScoreThreshold(roopfusion.globals.face_detector_score)
	with thread_semaphore():
...
def create_faces(vision_frame : VisionFrame, bounding_box_list : List[BoundingBox], face_landmark_5_list : List[FaceLandmark5], score_list : List[Score]) -> List[Face]:
	faces = []
	if roopfusion.globals.face_detector_score > 0:
		...
		iou_threshold = 0.1 if roopfusion.globals.face_detector_model == 'many' else 0.4
		...
		for index in keep_indices:
			...
			if roopfusion.globals.face_landmarker_score > 0:
				face_landmark_68, face_landmark_68_score = detect_face_landmark_68(vision_frame, bounding_box)
				if face_landmark_68_score > roopfusion.globals.face_landmarker_score:
...
def calc_embedding(temp_vision_frame : VisionFrame, face_landmark_5 : FaceLandmark5) -> Tuple[Embedding, Embedding]:
	...
	with conditional_thread_semaphore(roopfusion.globals.execution_providers):
...
def get_many_faces(vision_frame : VisionFrame) -> List[Face]:
	...
	try:
		...
		if faces_cache:
			...
			if roopfusion.globals.face_detector_model in [ 'many', 'retinaface']:
				bounding_box_list_retinaface, face_landmark_5_list_retinaface, score_list_retinaface = detect_with_retinaface(vision_frame, roopfusion.globals.face_detector_size)
			...
			if roopfusion.globals.face_detector_model in [ 'many', 'scrfd' ]:
				bounding_box_list_scrfd, face_landmark_5_list_scrfd, score_list_scrfd = detect_with_scrfd(vision_frame, roopfusion.globals.face_detector_size)
			...
			if roopfusion.globals.face_detector_model in [ 'many', 'yoloface' ]:
				bounding_box_list_yoloface, face_landmark_5_list_yoloface, score_list_yoloface = detect_with_yoloface(vision_frame, roopfusion.globals.face_detector_size)
			...
			if roopfusion.globals.face_detector_model in [ 'yunet' ]:
				bounding_box_list_yunet, face_landmark_5_list_yunet, score_list_yunet = detect_with_yunet(vision_frame, roopfusion.globals.face_detector_size)
		...
		if roopfusion.globals.face_analyser_order:
			faces = sort_by_order(faces, roopfusion.globals.face_analyser_order)
		if roopfusion.globals.face_analyser_age:
			faces = filter_by_age(faces, roopfusion.globals.face_analyser_age)
		if roopfusion.globals.face_analyser_gender:
			faces = filter_by_gender(faces, roopfusion.globals.face_analyser_gender)
