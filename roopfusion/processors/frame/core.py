...
from roopfusion.typing import ProcessFrames, QueuePayload
from roopfusion.execution import encode_execution_providers
...
def load_frame_processor_module(frame_processor : str) -> Any:
	try:
		frame_processor_module = importlib.import_module('roopfusion.processors.frame.modules.' + frame_processor)
...
def multi_process_frames(source_paths : List[str], temp_frame_paths : List[str], process_frames : ProcessFrames) -> None:
	...
	with tqdm(total = len(queue_payloads), desc = wording.get('processing'), unit = 'frame', ascii = ' =', disable = roopfusion.globals.log_level in [ 'warn', 'error' ]) as progress:
		progress.set_postfix(
		{
			'execution_providers': encode_execution_providers(roopfusion.globals.execution_providers),
			'execution_thread_count': roopfusion.globals.execution_thread_count,
			'execution_queue_count': roopfusion.globals.execution_queue_count
		})
		with ThreadPoolExecutor(max_workers = roopfusion.globals.execution_thread_count) as executor:
			...
			queue_per_future = max(len(queue_payloads) // roopfusion.globals.execution_thread_count * roopfusion.globals.execution_queue_count, 1)
